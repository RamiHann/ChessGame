...
BoardManager::BoardManager(const string &board): current_player(White) {
...
int BoardManager::move(const string &input) {
    int response;
    Location current;
...
int BoardManager::check_illegal_moves(const Location& current, const Location& destination) const {
    ...
    if (is_this_types(kings, current.x, current.y) &&
        !is_piece_threatened(get_current_king()->get_location()).empty()) {
...
int BoardManager::check_legal_moves(const Location& current, const Location& destination) {
...
vector<Location> BoardManager::is_piece_threatened(const Location& location) const {
    ...
    auto check_diagonal_threat = [&](int x_dir, int y_dir, bool check_pawn) {
        ...
        if (piece_in_way_location != CLEAR && is_this_types(diagonal, piece_in_way_location.x, piece_in_way_location.y))
            return true;
                 ...
                 is_this_types(pawns, piece_in_way_location.x, piece_in_way_location.y))
            return true;
        return false;
    };
...
void BoardManager::to_lower(string &str) const {
    string tmp = str;
    str.clear();
    for (char ch: tmp) {
...
Player BoardManager::get_player_color(const char &ch) {
    return (ch == '#') ? NoColor : islower(ch) ? Black : White;
}

template<class PieceType>
shared_ptr<PieceType> BoardManager::create_piece(const char &ch, Location starting_location) {
    return make_shared<PieceType>(get_player_color(ch), starting_location);
}

bool BoardManager::will_preform_castling(const Location &current, const Location &destination) {
    /*
    ...
    if (is_way_clear(current, Location((rook_x_place == BOARD_MIN_PLACE) ? rook_x_place + 1 : rook_x_place - 1,
                                       destination.y)) != CLEAR)
        return false;
    ...
    if (!dynamic_pointer_cast<King>(current_king)->is_first_move() ||
        !dynamic_pointer_cast<Rook>(current_rook)->is_first_move())
        return false;
...
Location BoardManager::is_way_clear(const Location &current, const Location &destination) const {
    /*
...
bool BoardManager::is_this_types(const string &types, const int &x, const int &y) const {
    return types.find(_board[x][y]->get_type()) != string::npos;
}

bool BoardManager::will_cause_checkmate() {
    /*
...
void BoardManager::change_places(const Location &current, const Location &destination, shared_ptr<Piece> destination_piece) {
    _board[current.x][current.y]->move(destination);
...
vector<shared_ptr<Piece>> BoardManager::get_all_pieces_of_player(Player player) const {
...
// ----------Algorithm--------------
pair<vector<pair<Location,Location>>,int> BoardManager::calculate_values(BoardManager board, Player current_player_color, const int &depth, const int &starting_depth) {
    if (depth < 0)
        ...
        for (auto move: all_moves) {
            ...
            if (!response) {
                ...
                if (response == LegalNextTurn || response == LegalCheck || response == Checkmate) {
                    ...
                    current_route.insert(current_route.end(), rest_route.first.begin(),
                                         rest_route.first.end() - good_route);
...
int BoardManager::threatened_by_weaker_piece(const Location &location) {
...
int BoardManager::threatening_stronger_piece(const Location &location) {
    /*
...
int BoardManager::get_piece_value(const Location &location) const {
...
shared_ptr<Piece> BoardManager::get_current_king() const {
    return (current_player == White) ? white_king : black_king;
}


ostream& operator<<(ostream& os, BoardManager& board) {
...
bool BoardManager::is_valid_location(const Location& location) {
    return location.x >= BOARD_MIN_PLACE && location.x < BOARD_MAX_PLACE && location.y >= BOARD_MIN_PLACE &&
           location.y < BOARD_MAX_PLACE;
}